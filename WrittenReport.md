...
## CONCLUSION
Reflecting on our project, one significant challenge we encountered was the limited range of Agent Ratings in our dataset, which spanned from 2.5 to 5.0, with the majority clustered around 4.0 and above. This skewed distribution meant that our predictions predominantly fell within this narrow range, resulting in a high accuracy that didn't feel entirely satisfying given the simplicity of the task.

To address this issue in future models, we could consider obtaining a more proportionate dataset, scaling the Agent Ratings differently, or imposing stricter penalties for incorrect predictions of the less frequent lower ratings. These adjustments could provide a more nuanced evaluation of the model's performance.

While we didn't expect our models to achieve perfect accuracy, we did anticipate a more varied distribution of prediction results. However, the delivery_time versus prediction rating graphs for models 2 and 3 revealed a conspicuous cutoff, indicating our model's inability to predict 5-star ratings accurately. This limitation likely stems from the common practice of assigning 5-star ratings by default, without substantive reasons, making it difficult for a machine learning model to discern meaningful patterns without sufficient data.

In hindsight, although our model performed reasonably well, the project highlighted areas for improvement. Future work could involve enhancing the sophistication of our model and addressing the inherent biases in the data. Additionally, exploring more advanced techniques and incorporating a broader range of features might yield more robust and reliable predictions.

Overall, this project has been a valuable learning experience. It underscored the importance of dataset quality and distribution in developing effective predictive models and provided insights into potential strategies for overcoming such challenges in future endeavors.
